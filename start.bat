...
set "WEB_PROCESS_HELPER=%ROOT%run scripts\newapi-web-process.ps1"
set "LIFECYCLE_WATCHER=%ROOT%run scripts\newapi-lifecycle-watch.ps1"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$watcher=$env:LIFECYCLE_WATCHER; $root=$env:PROJECT_ROOT; $script=$env:NEWAPI_SCRIPT; if (-not (Test-Path -LiteralPath $watcher)) { throw 'Lifecycle watcher not found: ' + $watcher }; $quote=[char]34; $quotedWatcher=$quote+$watcher+$quote; $quotedRoot=$quote+$root+$quote; $quotedScript=$quote+$script+$quote; Start-Process -FilePath powershell.exe -WindowStyle Hidden -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File',$quotedWatcher,'-OwnerProcessId',[string]$PID,'-Root',$quotedRoot,'-StartupScript',$quotedScript) | Out-Null; Write-Host 'Guarding. Press Ctrl+C to stop new-api and Docker Desktop.'; try { [Console]::TreatControlCAsInput=$true; while ($true) { if ([Console]::KeyAvailable) { $key=[Console]::ReadKey($true); if (($key.Key -eq 'C') -and (($key.Modifiers -band [ConsoleModifiers]::Control) -ne 0)) { break } }; Start-Sleep -Milliseconds 150 } } finally { try { [Console]::TreatControlCAsInput=$false } catch {}; if (Test-Path -LiteralPath $script) { $env:NEWAPI_NO_PAUSE='1'; & $script stop-all } else { Write-Host 'Startup script not found. Please stop services manually.' } }"
exit /b 0
