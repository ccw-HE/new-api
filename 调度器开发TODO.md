...
- 2026-07-04: 二次反馈后重新定位。当前 HEAD 为 `7b476bf9`，工作区干净；`/api/channel_scheduler/disabled`、`/api/channel-scheduler/disabled`、`/api/channel_scheduler/logs`、`/api/channel_scheduler/logs/stat` 均返回 401 非 404，说明调度器 API 路由存在。进一步探测发现真正稳定复现的 404 是渠道列表请求 `GET /api/channel?p=1&page_size=20`，而 `GET /api/channel/` 返回 401；准确位置为前端 `web/default/src/features/channels/api.ts` 的 `getChannels()` 请求与后端 `router/api-router.go` 渠道路由尾斜杠不兼容。
- 2026-07-04: 按 TDD 修复渠道列表根路径兼容：新增 `TestChannelListRouteAcceptsNoTrailingSlash`，先验证 `GET /api/channel` 与 `POST /api/channel` 缺路由导致失败，再在 `router/api-router.go` 为渠道列表/新增补充无尾斜杠路由。提交前验证：`go test ./router ./service ./model ./controller ./setting/operation_setting` 通过，`go build ./...` 通过，`git diff --check` 通过；重建 `new-api-dev` 后，3000/3001 上 `/api/channel`、`/api/channel/`、`POST /api/channel`、`/api/channel_scheduler/disabled`、`/api/channel-scheduler/disabled` 均返回 401 而非 404。
- 2026-07-04: 用户提供 Network 截图后确认真正根因是浏览器磁盘缓存：旧版 `/api/channel_scheduler/disabled` 曾走 `router/web-router.go` 的 API NoRoute 404，且全局 `middleware.Cache()` 给该 404 加了 `Cache-Control: max-age=604800` 与 `Cache-Version`，导致后续点击“调度器”直接命中旧 404 缓存。按 TDD 新增 `TestCacheMiddlewareDoesNotCacheApiResponses`，修复 `middleware.Cache()` 对 `/api`、`/v1`、`/mj`、`/pg` 返回 `no-store`；同时在 `web/default/src/lib/api.ts` 为 GET `/api*` 自动追加稳定 `_newapi_cache=<build revision>` 参数，绕开用户浏览器已有旧缓存。验证：`go test ./middleware ./router ./service ./model ./controller ./setting/operation_setting` 通过，`go build ./...` 通过，`npm run typecheck --workspace default` 和 `npm run build --workspace default` 通过，`git diff --check` 通过；重建 `new-api-dev` 后，用 `test/test1234` 登录，`/api/channel_scheduler/disabled?_newapi_cache=rv.0000.2k6e8r7p` 与 `/config?...` 返回 200，故意不存在的 `/api/definitely_missing_for_cache_probe` 返回 404 但缓存头为 `no-store, no-cache, must-revalidate, private, max-age=0`。
- 2026-07-04: 提交前小 review 补强缓存匹配边界：将后端 no-store 判断与前端 cache-bust 判断从简单前缀收窄为路径段匹配，避免未来 `/api-assets/...` 一类静态路径被误判为 API；补充静态路径仍返回 `max-age=604800` 的断言。
